...
function DSSM_Layer:__init(config)
   ...
   if self.dssm_type == 'hidden_network' then 
      ...
      self.master_network = self:new_temporal_hidden_network()
   end
end

-- network with hidden layer
function DSSM_Layer:new_hidden_network()
   print("Creating new hidden network for dssm")
  ...
  if self.gpu_mode then 
    print("Setting DSSM_Layer's network to cuda ")
    network:cuda()
  end
  return network
end
...
function DSSM_Layer:new_temporal_hidden_network()
   print("Creating new temporal network for dssm")
...
-- network with hidden layer
function DSSM_Layer:new_really_deep_hidden_network()
   print("Creating new really deep hidden network")
   local network = nn.Sequential()
      :add(nn.TemporalConvolution(self.in_dim, self.hidden_dim, 
        self.in_kernel_width, self.in_stride))
      :add(nn.Tanh())
      :add(nn.TemporalMaxPooling(self.hidden_kernel_width))
      :add(nn.TemporalConvolution(self.hidden_dim, self.out_dim, 
      self.hidden_kernel_width, self.hidden_stride))
      :add(nn.Tanh())
      :add(nn.Linear(self.out_dim, self.out_dim))   
...
function DSSM_Layer:new_network()
   print("Creating regular normal layer")
   local network = nn.Sequential()
